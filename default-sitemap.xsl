...
>
	...
	<xsl:template match="/">
		...
		<html xmlns="http://www.w3.org/1999/xhtml">
			<head>
				<title>
					<xsl:choose>
						<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">خريطة الموقع</xsl:when>
						<xsl:otherwise>فهرس خريطة الموقع</xsl:otherwise>
					</xsl:choose>
				</title>
		...
		.breadcrumb svg.back {
			...
			<body>
				...
				<xsl:call-template name="Header">
					<xsl:with-param name="title">خريطة الموقع</xsl:with-param>
	...
	<a href="/"><span>الرئيسية</span></a>
			...
			<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
					...
					<a href="/sitemap.xml"><span>فهرس خريطة الموقع</span></a>
						<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
	...
	<div id="content-head">
		...
		<xsl:choose>
			<xsl:when test="$fileType='RSS'">
				<p>تم توليده بواسطة <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank" rel="noreferrer noopener">All in One SEO</a>, this is an RSS Sitemap, meant to be consumed by search engines like Google or Bing.</p>
				<p>
			...
			<xsl:otherwise>
				<p>تم توليده بواسطة <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank" rel="noreferrer noopener">All in One SEO</a>, هذه هي خريطة موقع XML، والتي تهدف إلى استهلاكها بواسطة محركات البحث مثل Google أو Bing.</p>
				<p>
					يمكنك العثور على مزيد من المعلومات حول XML خرائط الموقع على <a href="https://www.sitemaps.org/" target="_blank" rel="noreferrer noopener">sitemaps.org</a>.				</p>
			</xsl:otherwise>
		</xsl:choose>
			...
			<p>
				<xsl:choose>
					<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">
						خريطة الموقع هذه تحتوي على						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								عنوان URL							</xsl:when>
							<xsl:otherwise>
								عناوين URL							</xsl:otherwise>
						</xsl:choose>
					</xsl:when>
					<xsl:otherwise>
						فهرس خريطة الموقع هذا يحتوي على 						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								خريطة الموقع							</xsl:when>
							<xsl:otherwise>
								خرائط الموقع							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
				وتم توليده في نوفمبر 4, 2024 الساعة 3:39 م			</p>
		</xsl:if>
	</div>
</xsl:template>
	...
	<xsl:variable name="priorityLabel">
		<xsl:choose>
			<xsl:when test="$priorityLevel = 'low'">منخفض</xsl:when>
			<xsl:when test="$priorityLevel = 'medium'">متوسط</xsl:when>
			<xsl:when test="$priorityLevel = 'high'">مرتفع</xsl:when>
		</xsl:choose>
	</xsl:variable>
	...
	<a href="/"><span>الرئيسية</span></a>
		...
		<h2 class="empty-sitemap__title">
			معذرة!			<br />
			لا يوجد مقالات هنا.		</h2>
		<div class="empty-sitemap__buttons">
			<a href="/" class="button">العودة إلى الصفحة الرئيسية</a>
					</div>
